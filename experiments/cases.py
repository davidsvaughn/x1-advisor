...
# Enumeration is the class §5.1/§7.3 singles out: its oracle is computed from
# the corpus, never written by the author, or v2 inherits v1's disease.
COMPUTED_ORACLE_CLASSES = {"enumeration_text", "enumeration_semantic", "known_absence"}
...
def load_fixtures(path: Path) -> dict[str, list[dict[str, Any]]]:
    ...
    if not isinstance(pools, dict):
        raise CaseValidationError([f"{path.name}: `pools` must be a mapping"])
    errors: list[str] = []
