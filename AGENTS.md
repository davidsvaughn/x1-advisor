...
| `docs/ADVISOR-V2-DESIGN-2026-08-21.md` | **proposed design + build plan** (2026-08-21) — ground-up v2 with fresh eyes over all three advisor generations: session-first substrate (event log, typed artifacts + handles, thread-scoped evidence with live-ACL revalidation, schema-driven thread brief) on Deep Agents/LangGraph with the X1 domain layer kept framework-free; lean QA loop; 3-week plan with a day-3 harness spike gate. Supersedes several prior decisions if adopted (its §9). **Nothing authorized — all David-gated** |
